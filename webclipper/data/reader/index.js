...
document.addEventListener("click", e => {
  const bol =
    e.target.dataset.cmd === "open-settings" ||
    Boolean(e.target.closest("#toolbar>div"));
  ...
  obj => {
    ...
    if (!article) {
      ...
      if (history.length) {
        ...
        window.alert(
          "Sorry the original content is not accessible anymore. Please load the origin content and retry"
        );
      }
    }
    ...
    stackedit.openFile({
      ...
      content: {
        text: "**" + article.title + "**\n" + article.content // and the Markdown content.
        // text: article.content // and the Markdown content.
      }
    });

    // Listen to StackEdit events and apply the changes to the textarea.
    stackedit.on("fileChange", file => {
      foo.value = file.content.text;
    });

    // automatically detect ltr and rtl
    [...iframe.contentDocument.querySelectorAll("article>*")].forEach(e =>
